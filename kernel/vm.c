...
{ // seperate function so we can keep track of depth using a parameter
  // Notes:
  // for each pte we need: VA, PTE bits, and PA
  // for a recursive call, we need to establish a base and recursive case
  //   -base case: if the pte is a leaf, we print VA, PTE bits, and PA for that pte
  //   -recursive case: if the pte is not a leaf, we recurse
  // very similar to a HW2 problem conceptually
  // VAs should match the example in the lab document
